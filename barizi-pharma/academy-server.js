    ...
    const response = await paystack.transaction.initialize({
      ...
      metadata: {
        custom_fields: [
          ...
          {
            display_name: "Program/Service",
            variable_name: "course",
            value: course || "General",
          },
...
app.get("/api/payment/verify/:reference", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[VERIFY] ref=${reference} | status=${data.status} | KES ${data.amount / 100}`,
    );

    const getField = (name) =>
      data.metadata?.custom_fields?.find((f) => f.variable_name === name)
        ?.value;
...
app.post("/webhook/paystack", (req, res) => {
  ...
  switch (event) {
    case "charge.success":
      ...
      console.log(
        `   Program : ${data.metadata?.custom_fields?.find((f) => f.variable_name === "course")?.value}`,
      );
  ...
  try {
    ...
    res.json({
      ...
      data: response.data.map((t) => ({
        ...
        channel: t.channel,
        paid_at: t.paid_at,
        program: t.metadata?.custom_fields?.find(
          (f) => f.variable_name === "course",
        )?.value,
      })),
    });
  } catch (err) {
...
app.post("/api/submissions", async (req, res) => {
  ...
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return res.status(500).json({
      success: false,
      message:
        "Supabase is not configured yet. Add your URL and service-role key to the environment.",
    });
  }
